...
### Lesson 4
# Task 1
#x = subset(Prefix.ver, SemanticClass=="opaque")[2, "LogFrequency"]
x = Prefix.ver[Prefix.ver[,"SemanticClass"] == "opaque", "LogFrequency"]
y = Prefix.ver[Prefix.ver[,"SemanticClass"] == "transparent", "LogFrequency"]
...
# check by Signs criterion
n = length(z)
s = sum(ifelse(z > 0, 1, 0))
s
#alpha_0 = 2^(-n) * sum(choose(n, s:n))
alpha_0 = 1 - pbinom(s - 1, n, prob=0.5)
alpha_0

# check by Wilcoxon signed rank test
# check simetry
z = sort(z)
half_n = as.integer(n / 2)
med = median(z)
ksi = med - z[1:half_n]
eta = z[n - 1:half_n + 1] - med
plot(ksi, eta)
abline(0, 1)
